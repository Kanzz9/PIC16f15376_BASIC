...
# 2 "<built-in>" 2
# 1 "main.c" 2
# 1 "E:\\microchip\\xc8\\pic\\include\\c99\\string.h" 1 3



# 1 "E:\\microchip\\xc8\\pic\\include\\c99\\musl_xc8.h" 1 3
# 4 "E:\\microchip\\xc8\\pic\\include\\c99\\string.h" 2 3
...
# 10 "E:\\microchip\\xc8\\pic\\include\\c99\\string.h" 2 3
# 25 "E:\\microchip\\xc8\\pic\\include\\c99\\string.h" 3
...
# 411 "E:\\microchip\\xc8\\pic\\include\\c99\\bits/alltypes.h" 3
typedef struct __locale_struct * locale_t;
# 25 "E:\\microchip\\xc8\\pic\\include\\c99\\string.h" 2 3
...
char *strerror (int);
# 65 "E:\\microchip\\xc8\\pic\\include\\c99\\string.h" 3
...
void EE_Write(uint16_t addr, uint8_t data)
{
    ...
    SW_I2C_WriteNBytes(0b01010000, &frame[0], 3);
...
uint8_t EE_Read(uint16_t addr)
{
    ...
    frame[0] = (uint8_t) (addr >> 8);
    frame[1] = (uint8_t) addr;
    SW_I2C_WriteNBytes(0b01010000, &frame[0], 2);
    SW_I2C_ReadNBytes(0b01010000, &frame[2], 1);

    return frame[2];
}

void EE_WritePage(uint16_t addr, const uint8_t *pdata)
{
    uint8_t frame[66];
    ...
    do { LATBbits.LATB0 = 0; } while(0);
    SW_I2C_WriteNBytes(0b01010000, &frame[0], 66);
    _delay((unsigned long)((5)*(32000000/4000.0)));
    do { LATBbits.LATB0 = 1; } while(0);
}

void EE_ReadPage(uint16_t addr, uint8_t *pData)
{
    uint8_t frame[2];
    ...
    SW_I2C_WriteNBytes(0b01010000, &frame[0], 2);
    SW_I2C_ReadNBytes(0b01010000, pData, 64);
}

void RandomArray(uint8_t *pD, uint8_t len)
{
    while (len > 0) {
        *pD = (uint8_t) rand();
        pD++;
...
_Bool MemoryTest(uint16_t addr, const uint8_t *pdata1, const uint8_t *pdata2, uint8_t len)
{
    uint8_t i;
    _Bool rslt = 1;
