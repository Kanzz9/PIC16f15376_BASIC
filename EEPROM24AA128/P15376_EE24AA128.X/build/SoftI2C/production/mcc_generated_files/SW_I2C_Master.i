# 1 "mcc_generated_files/SW_I2C_Master.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "mcc_generated_files/SW_I2C_Master.c" 2
...
# 1 "mcc_generated_files/SW_I2C_Master.h" 1





# 1 "mcc_generated_files/SW_I2C_Master_Cfg.h" 1
# 6 "mcc_generated_files/SW_I2C_Master.h" 2
...
int8_t SW_I2C_MasterOperation(_Bool read);
void SW_I2C_SetBuffer(void *buffer, size_t bufferSize);
# 2 "mcc_generated_files/SW_I2C_Master.c" 2
...
# 119 "mcc_generated_files/SW_I2C_Master.c"
static sw_i2c_cxt_t Sw_I2c_Cxt={.State=(-1)};
...
void SW_I2C_Init(void)
{
    ...
    do { ODCONCbits.ODCC4 = 1; } while(0);
    do { ODCONCbits.ODCC6 = 1; } while(0);
    do { TRISCbits.TRISC4 = 1; } while(0);
    do { TRISCbits.TRISC6 = 1; } while(0);
}

static void SW_I2C_Drv_Open(void)
{
                         ;
                                ;
    do { LATCbits.LATC4 = 1; } while(0);
...
static void SW_I2C_Drv_Close(void)
{
    _delay((unsigned long)((16)*(32000000/4000000.0)));
    do { LATCbits.LATC4 = 1; } while(0);
    do { LATCbits.LATC6 = 1; } while(0);
    do { TRISCbits.TRISC4 = 0; } while(0);
    do { TRISCbits.TRISC6 = 0; } while(0);
...
static void SW_I2C_SCL_Pulse(_Bool x)
{
    if(x)
    {
        do { TRISCbits.TRISC4 = 1; } while(0);
...
static void SW_I2C_SDA_Pulse(_Bool x)
{
    if(x)
    {
        do { TRISCbits.TRISC6 = 1; } while(0);
...
static void SW_I2C_WriteBit(_Bool x)
{
...
static _Bool SW_I2C_ReadBit(void)
{
...
static void SW_I2C_SendAck(_Bool ack)
{
    if(ack)
        SW_I2C_WriteBit(0);
    else
...
static uint8_t SW_I2C_Read(_Bool ack)
{
...
static _Bool SW_I2C_Write(uint8_t data_out)
{
    uint8_t i;

    for(i=0; i<8; i++)
    {
        SW_I2C_WriteBit((_Bool) (data_out&0x80));
        data_out<<=1;
    }

    return (SW_I2C_ReadBit()==0?1:0);
}

static void SW_I2C_Tasks(void)
{
    switch(Sw_I2c_Cxt.DoNext)
    {
        ...
        case SW_I2C_START:
            ...
            if(SW_I2C_Write(Sw_I2c_Cxt.SlvAddr)==0)
            {
        ...
        case SW_I2C_WRITE:
            if(SW_I2C_Write(*Sw_I2c_Cxt.Tx.pD)==1)
            {
                ...
                {
                    Sw_I2c_Cxt.SlvAddr>>=1;
                    Sw_I2c_Cxt.DoNext=SW_I2C_START;
                                            ;
                }
                else if(Sw_I2c_Cxt.State!=(-1))
                {
                    Sw_I2c_Cxt.State=0;
...
void SW_I2C_ReadNBytes(uint8_t address, uint8_t *data, size_t len)
{
    ...
    SW_I2C_Start();
    SW_I2C_Write((uint8_t) (address<<1)|1);
...
void SW_I2C_WriteNBytes(uint8_t address, uint8_t *data, size_t len)
{
    ...
    SW_I2C_Start();
    SW_I2C_Write((uint8_t) (address<<1));
...
int8_t SW_I2C_MasterOperation(_Bool read)
{
    if(Sw_I2c_Cxt.Buf.pD!=((void*)0))
    {
        if(read)
        {
            Sw_I2c_Cxt.Tx.pD=((void*)0);
            Sw_I2c_Cxt.Tx.Len=0;
