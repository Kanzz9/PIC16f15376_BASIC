...
void I2C_Init(void){
    // enable I2C1 ,config SCL,SDA as serial port pin 
    I2C1CONbits.ON = 1;
    // Slew rate control is disabled
    I2C1CONbits.DISSLW = 1;
    // I2CxADD is a 7-bit slave address
    I2C1CONbits.A10M = 0;
    //Acknowledge Data bit :Sends ACK during Acknowledge
    I2C1CONbits.ACKDT = 0;
    // enable MSSP port
}
...
uint8_t I2C_Scan(void){
    ...
    for(uint8_t i=0; i<=127; i++){
        ...
        ACK_bit = I2C1STATbits.ACKSTAT  ;  // Ack bit will come back low when the write is complete
...
{
    I2C1CONbits.SCIE = 1;          // clear SSP interrupt bit
    SBUF = databyte;              // send databyte
    while(!I2C1CONbits.SCIE);    // Wait for interrupt flag to go high indicating transmission is complete
}
